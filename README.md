...
| [Attention Residuals:把注意力「轉 90 度」用在網路深度上](./technology/llm-internals/architecture/attention-residuals.md) | Kimi 對「層」做注意力;**⭐ 殘差其實有「兩個」問題:數值膨脹 + 資訊稀釋**;每層配可訓練的 pseudo query 做點積+softmax,**權重總和 100% 治膨脹、加權側重治稀釋**,分工靠正向回饋自己長出來;**⚠️ 真正的瓶頸是「保存並讀取每層輸出」的顯存頻寬,不是 FLOPs** ⇒ 分塊(塊內普通殘差、塊間注意力殘差)|
...
| [ChatGPT 瀏覽器擴充功能:借用你「已經登入」的瀏覽器,在背景跨分頁做事(Gary Chen)](./technology/ai-productivity/chatgpt-browser-extension-agent.md) | 直接沿用現有瀏覽器 Profile(公司後台/Gmail/Drive 都進得去),**自建 Tab Group 在背景做事不搶畫面**;⭐ **三種入口分流:有 Plugin 就用 Plugin(API+結構化資料最省 token)、沒 Plugin 有網頁版用擴充功能、想隔離用內建 Browser**;**⭐ 比對官方文件補正四處:支援五種瀏覽器非只 Chrome(Opera 無 side chat)、有 side chat、安裝在 Settings>Computer Use、可取 YouTube 時間戳逐字稿**;**⚠️⚠️ 影片沒提的安全面——官方要你「把頁面內容當成不可信 context」(prompt injection,而 agent 握著你所有登入身分)**;兩原則:不可逆操作停在確認畫面、敏感任務用獨立 Profile |
...
| [AI 額度老是不夠用?三招省 Token:丟掉、縮減、打折(Gary Chen)](./technology/ai-productivity/token-saving-three-moves-context-control.md) | 每次送出重打包四層 context(**MCP 工具說明就先吃掉幾千 token**);換任務就開新對話、rewind 而非「我剛打錯」、先搜尋再餵、給結論清掉討論過程;**⭐ 比對官方文件補正三點:快取預設 5 分鐘非 1 小時、寫入要加價 1.25×/2×、`tools→system→messages` 階層(中途動 MCP 整包快取重算)** |
