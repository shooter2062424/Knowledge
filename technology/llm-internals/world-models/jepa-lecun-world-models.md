...
- **機器人抓取/操作(VJEPA 2 的主場):** 給定「把杯子移出平台」的目標狀態圖 → 編碼成 embedding → 在學到的世界模型上 **搜尋各種假設動作序列**,挑出能讓預測未來逼近目標的那組去執行。比起逐像素生成未來影格,JEPA 只預測 **顯著特徵的 embedding**,更穩、更可規劃。
- **自駕/行車預測:** LeCun 的例子——生成式模型會把算力浪費在預測「路邊樹葉的隨機晃動」(不可預測卻佔大量像素);JEPA 在 embedding 空間只關心會影響決策的東西(車、行人、路況)。
- **自監督視覺表示(已落地):** DINO V3 證明可在 **無人工標註** 下學到接近弱監督的影像表示,直接拿 patch embedding 做 **分割/檢索**——適合標註成本高的醫療影像、衛星影像等。
- **反例(別用 JEPA):** 純語言生成、寫作、對話——這是 LLM 的主場;JEPA 不生成文字。兩者「初期解不同問題」。

> 對 agent 工程的啟示(呼應 [[12-factor-agents]]、[[ai-harness-explained]]):若要 agent 可靠,LeCun 主張它得能 **預測自身行動的後果**、把推論變成「搜尋」而非單純自回歸——這正是世界模型補 LLM 的地方。
