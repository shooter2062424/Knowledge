...
- **為什麼 slash 一定排隊等回合結束**(時刻 1d 的表格那一列):它不是「給模型的訊息」而是「改 Claude Code 自己的設定/狀態」,中途注入(路徑 B)明確把它排除(`query.ts:1570` 的 `.filter(cmd => isSlashCommand(cmd) ? false : ...)`)。
- **為什麼這樣才對**:`/model` 改的是「下一個回合用哪個模型」這種全域狀態,如果硬塞進當前回合當文字,模型只會困惑「你叫我 /model 幹嘛」。把它隔離成獨立流水線,你打的 prompt 才永遠優先於這類設定指令。
- **skill 與 slash 是親戚**:時刻 8 會看到,**一個 skill 本質上就是一個 `type:'prompt'` 的 Command**,跟 slash command 共用同一套 `Command` 抽象與 `findCommand` 查找——差別只在 skill 會被模型自動觸發,slash 由你手動打。
...
## 時刻 3|送 API 之前的「壓縮閘」(對話還短,先空轉著)

`messagesForQuery` 在送出前會依序過四關:`applyToolResultBudget`(限制工具結果總量)→ `snip` → `microcompact` → `autocompact`。第一圈對話很短,這四關全是 no-op。

> **像機場安檢**:人少時你幾乎是直接走過去(no-op);要等人潮(對話 token)累積到一定程度,完整的檢查流程才會啟動。先記住這道閘**卡在「每圈送 API 前」這個位置**,它的精妙等對話變長(時刻 11)再看。
...
**咬合關係**:hook 跑在「時刻 5 的權限階梯」與「時刻 6 的並行判定」之間——PreToolUse 改完 `updatedInput` 後,時刻 6 的 `isConcurrencySafe(input)` 是**基於改過的 input** 判斷的。所以一個把 `ls` 改寫成 `rm` 的 hook(別這樣做)會連帶讓它從「可並行」變「不可並行」。

> **想改 hooks**:hook 的註冊與分派在 `utils/hooks/`;PreToolUse 影響權限的接點在 `permissions.ts`(hook 回的 `permissionDecision` 會在工具自己的 `checkPermissions` 之前被採用)。對 API error / prompt-too-long **不要**掛會阻擋的 hook(時刻 12 會講為什麼會無限燒 API)。
...
模型決定「逐一改 20 個測試檔」太佔對話,派一個子 agent。**最關鍵的設計決策**:子 agent **沒有自己的 harness,它用的就是主迴圈那個 `query()`**(時刻 0–12 那一整套,原封不動再跑一遍):
...
| 模組 | 它是什麼 / 在哪個時刻的旁邊 | 關鍵檔 |
|---|---|---|
| **Bootstrap / 啟動流程** | `query()` 跑之前的前置站:讀 settings、組初始工具池、啟動 MCP server、初始化 AppState。時刻 0 的 QueryGuard 其實是這之後才上場 | `setup.ts`、`bootstrap/` |
| **AppState(全域狀態袋)** | 工具、權限 context、tasks、MCP 連線狀態都掛在這個 store 上,靠它在主執行緒與子 agent 之間傳遞(時刻 10 的 `setAppStateForTasks` 就是它) | `state/AppState.ts` |
| **Slash commands 體系** | 時刻 1f 講過:`/model`、`/compact`… 走 `processSlashCommand`、不進 `query()` | `commands/`、`commands.ts` |
| **Hooks 體系** | 時刻 5b 講過:PreToolUse/PostToolUse/PostToolUseFailure + Stop hook(時刻 12) | `utils/hooks/`、`query/stopHooks.ts` |
| **Output Style** | 不只改身份措辭(時刻 2),還能**整段移除 `# Doing tasks`** 來重塑行為;它是 system prompt 的動態段之一,所以不破壞 boundary 前的全域快取 | `outputStyles/`、`constants/prompts.ts` |
| **Thinking / 推理預算** | 模型串流時的 thinking 佔 output budget(影響 `maxOutputTokens`);壓縮時(時刻 11)會**關掉 thinking** 省錢——這跟 token 預算故事同源 | `utils/thinking.ts` |
| **Cost / Usage 追蹤** | 每圈收集 `BetaUsage`(含 cache_read / cache_creation 拆分),是「為什麼 Claude Code 又省又快」的數據面,也是 CCB `token-budget`(時刻 11)判斷「燒超預算沒」的依據 | `cost-tracker.ts`、`costHook.ts` |
| **TodoWrite / Task 生命週期** | 背景 agent 與 bash task 的註冊/查詢/中止(時刻 10 的殭屍防護就靠它) | `Task.ts`、`tasks.ts`、`tools/TaskCreateTool` |
| **會話歷史 / `--continue`** | 對話按「專案路徑編碼」存進 `~/.claude/projects`,支援 `--continue` 接回(搬目錄會斷,見本庫另一篇) | `history.ts` |
| **IDE 整合** | VS Code / JetBrains 走 MCP-ide transport(時刻 9 的 `sse-ide`/`ws-ide`),只放行 `mcp__ide__executeCode` / `getDiagnostics` 白名單 | `services/mcp/`(ide 分支) |
