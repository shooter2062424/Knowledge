...
>
> ⭐⭐⭐ **2026-09-21 再增補 Gary Chen**〈[爆紅新模型 Jev 解析,不聊天只做決定,快 200 倍又便宜 400 倍?](https://www.youtube.com/watch?v=2mtn-Qp59y4)〉(2026-09-21,約 13.7 分鐘,官方 zh-TW 字幕),成為 **§十五:自動化視角** —— 申論題 vs 選擇題的比喻、六個社群 Demo、一個完整的客服工作流設計(**讓 Jev 當發信前的最後把關者**)。⭐⭐⭐ **本節並補上一份獨立第三方評測:單問一題 Jev 62.6% 輸給 Haiku 81.3%,拆成五個窄信號後才以 95.0% 反超** —— **對 §14 心法一最有力的實證,也打臉「Jev 判斷一定更準」的直覺**(見 §15.6)。⚠️ 該片作者推廣自家 Skool 社群內容與提示詞工具。
>
> ⭐⭐⭐ **2026-09-26 再增補 Why QQ**〈[快200倍!便宜400倍!给Coding Agent加决策层 原理和实战](https://www.youtube.com/watch?v=ou9SC0Z_CtI)〉(2026-09-25,約 9.6 分鐘,官方 zh-Hans 字幕),成為 **§十六:接進 coding agent** —— token 大多花在「讀」而非「寫」、**長對話切換模型反而更貴**、四個用法。⭐⭐ **已 clone hermes-jev-skills 讀 CHANGELOG,補上兩段比影片更有價值的內容**:一次**影子模式裡 89% 回合被送去最貴檔**的失敗與修正(**不確定的 score 平均值會落在門檻上**),以及交接實驗的完整數據(**「留原文 + 能搜」勝過任何摘要**)。⚠️ 影片把 hermes 的 0.00006 美元誤掛在 LangChain 名下,另有一組 LangChain 數字在部落格中查無。
...
## 十五、⭐⭐⭐ 自動化視角:把它當工作流裡的一個零件,與一份打臉又佐證的第三方評測(2026-09-21 增補,來源:Gary Chen)

> **§十四講「怎麼用對」,這一節補上**為什麼自動化工作者特別該關心它**,
> 以及一個完整的客服工作流設計。**
>
> ⭐⭐⭐ **本節最有價值的是 §15.6:一份獨立第三方評測 ——
> **它先打臉了「Jev 一定比小模型準」的直覺,然後用數據佐證了 §14 心法一的正確性。****
>
> ⚠️ **立場揭露:該片作者推廣自家 Skool 社群的付費/會員內容與提示詞工具,本文不轉述那些素材。**

---

### 15.1 ⭐⭐⭐ 最好懂的一個比喻:申論題 vs 選擇題

| | **一般大模型** | ⭐ **Jev** |
|---|---|---|
| **像在寫** | **申論題** | **選擇題** |
| **過程** | ⚠️ **一句一句慢慢寫,寫得慢,還可能離題** | ⭐ **只要圈一個答案** |
| ⭐⭐⭐ **所以** | — | **速度極快,而且答案絕對不會發散 —— 一定只會是你給的選項之一** |

> ⭐⭐⭐ **「這也是為什麼 Jev 能做到低幻覺:**它沒有機會去瞎掰一個選項以外的答案出來**。」**

📎 **這與本文 §五的結論一致:**它保證的是「格式永遠合法」,不是「判斷永遠正確」**。**

---

### 15.2 ⭐⭐ 為什麼自動化工作者該特別關心

> ⭐⭐⭐ **「現在大家用 AI 搭出來的自動化流程裡,**有一大堆步驟其實只是在做判斷**。」**
...
> ⚠️⚠️ **「但我們現在卻都習慣請**又貴又慢的大模型**來處理 —— 這其實非常浪費時間和金錢。」**
>
> ⭐⭐ **「它最適合的位子,是放在你的工作流裡面**當一個決策節點**。」**

---

### 15.3 ⭐ 三種題型的白話版(⚠️ 並補正一處規格)

| 題型 | 白話 | ⭐ **官方規格** |
|---|---|---|
| **是非題(`noul`)** | **問它一句話是不是真的,回 0 到 1 的機率** | — |
| **單選題(`choice`)** | **從你列的選項挑一個,同時告訴你每個選項的機率** | ⭐ **最多 255 個選項**,✅ **與影片一致** |
| **打分題(`score`)** | **先定好一把尺(光譜),寫出每個級距代表什麼** | ⚠️⚠️ **官方文件寫的是 2–10 級** |

> ⚠️⚠️ **補正:影片說打分題「最多可以分到 11 級」,⭐ 但官方文件寫的是 **2–10 levels**。**
> **要設計刻度時以官方為準。**

⭐ **影片補了一個很實用的細節:`choice` 的每個選項**還可以搭配一句簡單的說明**,讓它更清楚這個選項的定義 ——
📌 **這與 §14.1 官方請求範例裡的 `criteria` 欄位對得上。**

**⭐ 三種題型各自的自動化範例(影片版,很具體):**

| 題型 | 範例 |
|---|---|
| **是非題** | **丟發票問「是不是詐騙」→ 回 90%;⭐ 把常見詐騙特徵寫進條件再問一次,信心水準會更高** |
| **單選題** | **自動整理信箱,判斷每封信該歸到哪個資料夾** |
| ⭐ **打分題** | **業務名單分級:0 = 沒預算的學生、1 = 小團隊、2 = 有明確需求與時程的公司、3 = 預算充足的大企業 → 大企業表單進來打 2.9 分 → **2.5 分以上觸發緊急通知給業務主管**** |

---

### 15.4 ⭐⭐ 社群玩法(⚠️ 皆為社群自述,未核實)

| # | 誰 | 做了什麼 |
|---|---|---|
| **①** | **Matthew Berman** | ⭐⭐ **虛擬小鎮 50 個 AI 居民**:廣播「麵包店大拍賣」後 **0.6 秒內**各自決定反應 —— **39 個不為所動、6 個去看看、4 個加入搶購、1 個跑去通知別人** |
| **②** | **AI ML API 團隊** | ⭐⭐⭐ **西洋棋快棋對弈**(見 §15.5) |
| **③** | **壓力測試** | **1,000 封 Email 分類:Jev 跑完 70 秒、成本 9 美分;⚠️ 同一批交給 GPT-5.6 Luna 要 5 分鐘、62 美分** |
| **④** | **Ryan Vogel(OpenCode 創始團隊)** | **1,700 封信分類 + 排優先序 + 垃圾信分數 + 判斷要不要親自回覆,總共 18 美分** |
| **⑤** | **Riley Brown** | ⭐ **模型路由器**:Jev 自己不回答,只判斷這題該派給哪個模型(改錯字 → 最便宜的;寫複雜程式 → 最貴最聰明的) |
| **⑥** | **kitze** | **Unclutter 瀏覽器外掛**:網頁一開就在背景呼叫 Jev,即時判斷哪些區塊是廣告、哪些是沒營養的內容,瞬間清乾淨 |

### 15.5 ⭐⭐⭐ 那場西洋棋:最聰明的模型輸在想太久

**規則:每方只有五分鐘的快棋,每下一步等於呼叫一次模型。**

| 對手 | 結果 |
|---|---|
| ⚠️ **vs Fable** | **單論棋力 Fable 完全碾壓,第 29 步已佔盡優勢、還多吃一隻皇后 —— ⚠️⚠️ **但 Fable 每步要想 6–15 秒,Jev 每步只花 2.6 秒**;最後 Fable **五分鐘耗光、超時判輸**** |
| ⚠️⚠️ **vs Astra** | **Astra 只用 18 步就把 Jev 將軍死棋** |

> ⭐⭐⭐ **這兩局合起來才是完整的故事:**
> **「最聰明的模型,最後卻輸在想太久」是真的;**
> ⚠️ **「但換一個對手,Jev 18 步就被打死」也是真的。**
> **⭐ 速度不能替代能力 —— 它只是在**時間也是資源**的場景裡成為一個變數。**

📎 **這正好呼應本文 §十三的座標:**它對標的是 Flash/nano 那一檔,而且只在工作流特定任務上**。**

---

### 15.6 ⭐⭐⭐ 一份獨立第三方評測:先打臉,再佐證

**⭐⭐ 本文查到一份公開的獨立評測,用的是 2,000 封釣魚郵件測試集
(PhishNChips v5.2,⚠️ **郵件本文為合成、標籤來自 URL 信譽來源**)。**

#### ⚠️⚠️⚠️ 第一個結果:單問一題,Jev 輸給 Haiku

| 做法 | Jev | Haiku |
|---|---|---|
| ⚠️ **只問一個總問題「這是不是釣魚信」** | ⚠️⚠️ **62.6%** | **81.3%** |
...
> ⚠️⚠️⚠️ **這直接打臉一個很常見的直覺:**「Jev 專做判斷,所以判斷一定比小模型準」——不成立。**

#### ⭐⭐⭐ 第二個結果:拆成五個窄信號之後,反超

| 做法 | Jev | Haiku |
|---|---|---|
| ⭐⭐⭐ **拆成五個針對性信號**(例:連結是不是短網址或免費空間、寄件人是否用免費信箱卻自稱是某組織),**再用邏輯斯迴歸在 1,000 封上擬合權重、在另外 1,000 封上評分** | ⭐ **95.0%** | **93.2%** |

> ⭐⭐⭐ **這是本文目前找到**對 §14 心法一最有力的實證**:**
> **「一個問題只問一件事,複雜判斷拆成原子問題,再在程式碼裡加權組合。」**

#### ⚠️⚠️ 但評測作者的告誡同樣重要,本文照錄

> ⚠️⚠️⚠️ **「Jev 在兩者都只被問一個問題時**輸給了 Haiku**。
> 它是在**一個人把任務拆成五個窄信號、並用有標註的資料擬合權重之後**,才追平 Haiku。」**

**⭐ 也就是說,那 95% 不是「換上 Jev 就有」的:**

| 代價 |
|---|
| ⭐ **要有人**想清楚該拆成哪五個信號** |
| ⚠️ **要有**標註資料**來擬合權重** |
| ⚠️ **要多跑五次判斷**(雖然 §14.4 說明了合併成一次請求後成本極低) |

> ⭐⭐⭐ **本文的結論:**Jev 不是一個「更準的分類器」,它是一個「更便宜的原子判斷單元」。**
> **準確度來自**你怎麼設計那組原子問題與組合邏輯**,不是來自模型本身。**

📎 **這與本文 §12.5 的補正、§14.4 的心法一完全一致,可以放在一起讀。**

---

### 15.7 ⭐⭐ 一個完整的客服工作流設計(影片最實用的一段)

**場景:一張客訴單寫著「我被重複扣款兩次,今天一定要給我處理好!」
同時後台顯示這位客人用的是高階付費方案、帳號已使用四百多天。**

#### ⭐⭐⭐ 把單子與背景資料一起丟給 Jev,**同時拋出五個問題**

| # | 問題 | 題型 |
|---|---|---|
| **①** | **這張單的分類是什麼?**(帳務問題 / 系統故障 / 其他) | **choice** |
| **②** | **嚴重程度 0 到 10 分是幾分?** | **score** |
| **③** | **客人有沒有要求退款?** | **noul** |
| **④** | **客人有沒有具體寫出問題發生的操作步驟?** | **noul** |
| **⑤** | **客人現在的情緒狀態?**(平靜 / 不耐煩 / 憤怒) | **choice** |

> ⭐ **「這五個關鍵問題的答案,**不到一秒鐘就能全部生成出來**。」**
> **接著串接到預先寫好的分類邏輯,自動派送到帳務部門,甚至直接亮紅燈提醒主管。**

📌 ⭐⭐ **這正是 §14.4 心法二「投機扇出」的標準用法 —— 一次請求塞五個問題,而不是呼叫五次。**

#### ⭐⭐⭐ 然後是全片最聰明的一個設計:讓 Jev 當**最後的把關者**

```mermaid
flowchart LR
    A["<b>Jev</b><br/>五個問題<br/>分類與派單"] --> B["<b>大模型</b><br/>代筆寫安撫信"]
    B --> C["⭐⭐⭐ <b>發送前再呼叫一次 Jev</b><br/>當最後的把關者"]
    C --> D["有沒有真的<br/><b>回答到客人的痛點</b>?"]
    C --> E["語氣像<b>誠懇道歉</b><br/>還是<b>推卸責任</b>?"]
    C --> F["⚠️ 有沒有不小心把<br/><b>公司內部機密規定洩漏出去</b>?"]
```

> ⭐⭐ **「加上 Jev 這道檢查關卡,不僅速度快、成本幾乎可以忽略不計,
> 還能大幅降低 AI 產生幻覺亂回覆客人的**公關危機風險**。」**

📎 ⭐⭐⭐ **這與本庫 [[agent-paid-api-cost-guardrails-mcp]]、
[[cloudflare-security-audit-skill-pipeline]] 的「驗證者不能是產出者」是同一個原則的不同尺度 ——
**寫信的是大模型,把關的是另一個便宜得多、而且不會瞎掰的模型。****

---

### 15.8 ⭐ 對開發者的一個具體建議

> ⭐⭐ **「它非常適合被放在**檢查的關卡**,幫你掃描註解、審查程式碼邏輯 ——
> 因為它實在太便宜了,**便宜到你每一次存檔、每一次改 code 都可以毫無壓力地讓它跑一遍**。」**

⭐ **對照 §十一的傑文斯悖論:**便宜到一定程度,本身就是一種新能力** —— 這就是一個具體例子。**
...
| 項目 | 狀態 |
|---|---|
| ⚠️⚠️ **「Agent 自己挑 skill 出錯率 17%,先用 Jev 過濾降到 7.3%」**(影片稱為 TypeSafe 官方數據) | ⚠️⚠️ **本文遍尋官方文件與搜尋皆未找到這組數字,列為未核實** |
| **速度快 20–200 倍、價格便宜 40–400 倍** | ⚠️ **與其他來源的區間不同**(官方首頁 193.6× / 444.6×、Caleb 版 40–200×);⭐ **本文建議一律以「視工作負載而定的區間」理解,別引用單一數字** |
| **X 貼文近 3,000 萬次觀看、HN 1679 分 / 400+ 留言** | ⚠️ **未回溯原帖**;⚠️ **與 §八(Why QQ 版)的 1,777 讚不一致,兩者皆未核實** |
| **所有社群 Demo 的數字** | ⚠️ **皆為社群自述,未核實** |

**⭐ 但本節另外查到幾個**有公開複現說明**的社群評測數字,比影片的轉述更值得參考:**

| 評測 | 結果 |
|---|---|
| **工具呼叫風險分類(60 案例)** | **整體 91.7%(55/60);清楚案例 100%、對抗性案例 91.7%、⚠️ 模糊案例 71.4%** |
| ⭐ **Skill 路由(72 個合成請求)** | **Jev 94.4%(68/72) vs 詞彙比對基線 70.8%(51/72)** |
| **緊急程度分類(40 案例)** | **只錯一題** |

> ⚠️ **這些仍屬個人評測、樣本量小、無同儕審查** —— ⭐ **但至少有公開的方法說明與可複現的倉庫。**

---

### 15.10 ⭐⭐⭐ 一句話帶走

> ⭐⭐⭐ **「不要問現在哪個模型最強,你要問的是:**哪個模型最適合你當下的工作流**。」**
>
> ⭐⭐ **「就像好久以前大家就知道:用高階聰明的模型做 Planning,再用低階快速的模型去執行苦力實作。
> 而現在,我們的武器庫裡又多了一個**專職的決策模型**。」**

---

## 十六、⭐⭐⭐ 接進 coding agent:錢花在哪、路由什麼時候反而更貴,以及社群實測踩到的坑(2026-09-26 增補,來源:Why QQ)

> **前面幾節講 Jev 本身;這一節講**把它接進 Claude Code / Codex 這類 coding agent**。**
>
> ⭐⭐⭐ **本節已 `git clone` 社群專案 [kerpopule/hermes-jev-skills](https://github.com/kerpopule/hermes-jev-skills)(MIT,823 star)讀 README、`docs/` 與 CHANGELOG 核實**,
> 並讀 [LangChain 官方部落格](https://www.langchain.com/blog/building-a-harness-with-jev)。
> ⭐⭐ **CHANGELOG 裡有兩段比影片講的更有價值:**一次路由設計的失敗與修正**(§16.5),以及**交接實驗的完整數據**(§16.4)。

---

### 16.1 ⭐⭐⭐ coding agent 的 token 花在哪

**影片引用 TypeSafe 一份設計文件裡的子任務分布(⚠️ 本文未取得該文件原文核對):**

| 子任務 | 占 token |
|---|---|
| ⭐⭐ **讀檔案內容** | **30–40%**(最大宗,而且常反覆讀全文) |
| **搜尋、探索程式庫** | **10–18%**(grep 結果雜訊遠多於訊號) |
| **命令輸出** | **10–20%**(stack trace 和 log 又臭又長) |
| **系統提示 + 工具定義** | **5–12%**(固定開銷,每輪重複付) |
| ⚠️ **真正寫程式、改程式** | **只有 4–10%** |

> ⭐⭐⭐ **「你以為自己雇了個程式設計師,實際雇了個昂貴的圖書管理員。」**
> ⭐⭐ **「想省錢,得讓模型少讀廢話;光催它打字更快沒用。很多團隊盯著輸出側優化,方向從一開始就偏了。」**

📌 **影片另引微軟 fastcontext 專案:GPT-5.4 的軌跡裡,讀檔與搜尋占 56% 的工具呼叫輪次、吃掉主 agent 近一半 token。**(⚠️ 未核實)

---

### 16.2 ⚠️⚠️⚠️ 路由不等於省錢:長對話切換模型反而更貴

> **很多人以為「簡單的交給便宜模型、難的交給貴模型」一定省錢。**
> ⚠️⚠️ **影片引用的那份設計文件算了一筆帳(設上下文 X、生成 Y、工具回讀 Z):**

| 方案 | 成本 |
|---|---|
| **全程用貴模型** | **25Y + 5Z** |
| **中途切到便宜模型再切回來** | **3X + 20Y + 8Z** |

> ⭐⭐⭐ **會話一長、X 一大,切換反而更貴 —— 因為**切回貴模型時,它要把整個上下文重新讀一遍**。**
> **依文件的典型比例,全程用貴模型只要切換方案的三分之二價錢。**
...
> ⭐⭐ **Jev 剛好符合這個前提:它做路由判斷時只看**當前這一輪的幾百個字**,不背整個會話歷史 —— 所以它的路由判斷本身是便宜的。**
> ⚠️ **但這只說明「判斷便宜」;被路由到的那個模型若要重讀長上下文,上面那筆帳照樣成立。**

---

### 16.3 ⭐⭐ 四個用法(影片整理,⭐ 已對官方與 repo 核實)

| 用法 | 做法 | ⭐ **核實** |
|---|---|---|
| **① 模型路由** | **Jev 先判斷這則訊息的難度與類型,再從路由池挑最便宜夠用的模型** | ✅ **LangChain 的 `ModelRouterMiddleware`**;hermes 把路由池排成「難度 × 工作類型」的網格 |
| **② 上下文篩選** | **對每一輪問一個 `noul`:「這段留著有用嗎?」** | ✅ **hermes README:71 輪 0.95 秒;比「按時間保留最近的」贏 11 題對 4 題** |
| **③ 技能選擇** | **不把幾百個技能的完整定義全塞進系統提示** | ✅ **hermes README:377 個技能約 2.8 秒選完;寒暄類輪次在本地免費放行** |
| **④ 工具呼叫護欄** | **執行 bash 前先判斷危不危險** | ✅ **LangChain 的 `AutoModeMiddleware`** |

⚠️ **影片說 AutoModeMiddleware「單次檢查 0.00006 美元」—— LangChain 部落格**沒有**給這個數字。
⭐ 這個數字出自 hermes 的 `jev triage`(分類一則訊息「約 400 毫秒、0.00006 美元」),不是 LangChain 的 bash 檢查。**

⚠️ **影片說 LangChain 部落格給出「準確率 67.8%、與 GPT-5.6 Terra 打平、成本約 1/76」—— 本文讀部落格時**沒有找到這組數字**;
部落格只有「分類任務最快 200 倍、最便宜 400 倍」,並註明是「公司自行回報」。**

---

### 16.4 ⭐⭐⭐ 交接實驗:摘要寫得再好,都比不上「留原文 + 能搜」

**影片的說法是「讓 Jev 寫摘要再交接,召回率反而下降」。**
⭐ **本文讀 hermes CHANGELOG 後的精確版本(Jev 本身不會寫文字,寫摘要的是宿主的輔助模型):**

| 交接方式 | 召回率(7 段真實對話、104 題) |
|---|---|
| ⚠️ **依 Jev 的「保留 / 摘要 / 丟棄」標記產出的摘要** | **37.5%** |
| **直接拿最後 24,000 字元寫摘要** | **48.1%** |
| ⭐ **寫手讀**完整對話**、給 1,200 字額度** | **58.7%** |
| ⭐⭐⭐ **上一行 + 允許搜尋一次舊對話** | **75.0%** |
| ⭐ **完全不寫摘要、只允許搜尋一次舊對話** | **56.7%** |

> ⚠️ **為什麼 Jev 標記版最差?CHANGELOG 講得很誠實:**
> **「Jev 的標記確實比按時間挑選好(11 比 4),**判斷是真的**;
> 但圍繞它的摘要流程把其他每一輪都截到 400 字元,**這個代價比判斷賺到的還多**。」**

> ⭐⭐⭐ **最值得帶走的一條:「搜尋一次舊對話,讓每一種摘要都多了 16 到 33 分;
> **不寫摘要、只能搜一次,就贏過所有不能搜的摘要**。」**
>
> ⭐⭐ **影片的結論本文同意:「保留原文加檢索入口,比精心壓縮的摘要更能打。」**

📎 **這與本庫 [[supermemory-memory-layer]] 的「記憶與檢索要分開設計」、
[[agit-version-control-for-agent-sessions]] 的「直接交出整段對話,而不是交接文件」是同一個方向。**

---

### 16.5 ⭐⭐⭐ 一次真實的失敗:影子模式裡 89% 的回合被送去最貴那一檔

> **影片建議「先開影子模式(shadow),只記錄不切換,跑一天看日誌」。**
> ⭐⭐⭐ **hermes 的 CHANGELOG 記錄了為什麼這一步不能省 —— 他們自己就在影子模式裡抓到一個嚴重問題:**

> **「在 41 個設定檔的真實環境中以影子模式運行,**89% 被判斷的回合被送往困難檔**。
> 三個原因都**不是**因為那些回合真的很難。」**

| 原因 | 修正 |
|---|---|
| ⚠️⚠️ **不確定的 `score` 平均下來會落在量表中間,而中間剛好是「困難」的門檻** | ⭐⭐ **改讀 Jev 回傳的**各級機率**:困難需要 P(大量 或 專家)≥ 0.6;簡單需要 P(瑣碎)≥ 0.7。**不確定的回答永遠買不到最貴那一檔** |
| **風險關鍵字會把回合升到困難** | **改成只設「最低中等」的下限** |
| **長回合被整段拿去判斷** | **只看開頭與結尾(約 2,500 字元),風險字也只檢查這段** |
| **樣板類回合(交接、排程)也被路由** | **這類回合不路由,維持原本設定的模型** |

> ⭐⭐⭐ **修正後在 400 個真實回合上重放:困難檔從 **89% 降到 4.5%**。**

> ⭐⭐⭐ **本文認為這是整個 Jev 系列裡最實用的一條工程教訓:**
> **用 `score` 做門檻判斷時,**不要只看平均分數**——不確定時平均值會往中間靠,正好落在門檻上。
> 要讀各級的機率分布,並規定「不確定時該怎麼辦」。**

📎 **這正好印證了 §12.5 的補正:**校準是分布層級的性質**,單次的平均分數不能直接當決策依據。**
...
| 原則 | 內容 |
|---|---|
| ⭐⭐ **失敗開放(fail-open)** | **Jev 掛了、逾時、信心低 ⇒ 沿用目前的模型,**永遠不擋住對話**;每個子指令都會回傳一個可用的 fail-open 答案** |
| ⭐ **先影子、再開啟** | **`/jev routing shadow` 跑一天,看 `logs/jev-decisions.jsonl`,再 `/jev routing on`** |
| **風險回合不降級** | **帶「生產、刪除、付款」這類風險詞的回合,不往最便宜那一檔送**(⭐ §16.5 後改為「最低中等」) |

### ⭐⭐⭐ 寫問題時最值錢的一句話:講清楚什麼是「必要」、什麼只是「加分」

**✅ hermes 文件引用的案例:判斷一支影片是否符合指定主題。**

| 寫法 | 準確率(44 支人工標註影片) |
|---|---|
| **把主題、語氣、格式、受眾全寫進描述** | **16 / 44** |
| ⭐ **只把「主題」寫成必要條件,其他明寫成「只用來打破平手的偏好」** | ⭐ **33 / 44** |

> ⭐⭐ **原因:「Jev 無法知道你的描述裡哪一行才是硬條件,所以會把軟性屬性也當成篩選條件。」**
> **⭐ 規則:「永遠不要留一個沒標註的次要屬性 —— 沒標註就會被當成必要。」**

⚠️ **hermes 文件自己註明來源:這是**販售相關產品的廠商自行回報**,n=44,沒有逐案細節 ——
「把它當成值得試、值得量的預設做法,而不是定律」。**

📎 **這與 §14.4 心法一「一個問題只問一件事」、§15.6 第三方評測「拆成窄信號後才反超」是同一件事的第三個證據。**

---

### 16.7 ⭐⭐ 落地建議(影片)

> ⭐⭐⭐ **「別一上來就圍繞 Jev 重構 agent。」**

| 步驟 | 做法 |
|---|---|
| **①** | **找一個你現在靠慢速 LLM 呼叫撐著的判斷點,或一個老是寫錯的正則** |
| **②** | **給 Jev 最小的狀態、明確的候選** |
| ⭐ **③** | **把它的機率和你現有的結果**並排記一週** |
| **④** | **它證明自己配得上這個分支,再把更多判斷交過去** |

> ⭐⭐⭐ **「生成很貴,判斷很便宜 —— 別把兩件事交給同一個模型做。」**
> ⭐ **「Jev 可以被取代,但『決策層』這個設計思想會留下來。」**
...
- ⭐⭐⭐ [快200倍!便宜400倍!给Coding Agent加决策层 原理和实战 — Why QQ](https://www.youtube.com/watch?v=ou9SC0Z_CtI)(2026-09-25,約 9.6 分鐘,官方 zh-Hans 字幕;**§十六來源**)
- §十六的核實來源:
  - ⭐⭐⭐ [kerpopule/hermes-jev-skills — GitHub](https://github.com/kerpopule/hermes-jev-skills)(MIT,本文查詢時 823 star;**已 clone 讀 README、`docs/writing-a-jev-question.md` 與 CHANGELOG —— §16.4 交接實驗與 §16.5 路由失敗的依據**)
  - [Building a harness with Jev — LangChain Blog](https://www.langchain.com/blog/building-a-harness-with-jev)(`ModelRouterMiddleware`、`AutoModeMiddleware`、「最快 200 倍、最便宜 400 倍」為公司自行回報)


- ⭐⭐⭐ [爆紅新模型 Jev 解析,不聊天只做決定,快 200 倍又便宜 400 倍? — Gary Chen](https://www.youtube.com/watch?v=2mtn-Qp59y4)(2026-09-21,約 13.7 分鐘,官方 zh-TW 字幕;**§十五來源**。⚠️ 作者推廣自家 Skool 社群內容與提示詞工具,本文不轉述)
- §十五的核實來源:
  - ⭐⭐⭐ [TypeSafe's Jev Scores 62.6% Asked Once and 95% Split Five Ways — THE D*AI*LY BRIEF](https://www.beri.net/article/typesafe-jev-typed-decision-model-calibration-decomposition-shadow-eval)(**2,000 封 PhishNChips v5.2 釣魚郵件測試;⚠️ 郵件本文為合成、標籤來自 URL 信譽來源**)
  - [themsquared/jev-benchmark — GitHub](https://github.com/themsquared/jev-benchmark)(工具呼叫風險分類的可複現評測)
  - ⚠️ **§15.3 補正依據**:[Jev API 文件](https://jevapi.org/)明列 `score` 為 **2–10 levels**,影片說的 11 級與官方不符。
