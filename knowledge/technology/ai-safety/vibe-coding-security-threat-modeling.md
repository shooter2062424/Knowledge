...
**來源影片:** 兩支 Gary Chen 影片
- 〈[給非技術人員的資安教學,Vibe Coding 必學的基本功](https://www.youtube.com/watch?v=t9WA-BkLUps)〉(2026-08-29,約 22.7 分鐘,無字幕)—— §1–8
- 〈[Zeabur 出事,你今天該做這三件事](https://www.youtube.com/watch?v=ExWlrAQrfdc)〉(2026-08-30,約 5.9 分鐘,官方 zh-TW 字幕)—— §9 事後應變
**整理日期:** 2026-08-30(§9 於 2026-08-31 增補)

> ⚠️ **§1–8 的來源影片無官方字幕也無自動字幕**,逐字稿以 CPU 版 faster-whisper 轉錄取得,
> **非官方字幕**(專有名詞對照見文末)。**§9 的來源影片有官方 zh-TW 字幕,無此問題。**
> Codex Security Plugin 的可查證規格**已比對 OpenAI 官方文件與 `openai/codex-security` 倉庫核實**(補正見 §6);
> §9 的 Zeabur 事件細節**已比對多家媒體報導核實**。
...
## 9. ⭐⭐ 事後應變:平台真的出事時該做什麼(2026-08-30 新增)

> 本節整理自 Gary Chen〈Zeabur 出事,你今天該做這三件事〉(2026-08-30,約 5.9 分鐘,官方 zh-TW 字幕)。
> 作者開頭就說,前一支資安影片才剛聊完,隔天就爆出這件事 —— 這一節正好是本文前八節的**事後對照組**:
> 前面講「上線前怎麼建模」,這裡講「**別人出事而你被波及時怎麼止血**」。

### 9.1 事件本身(已比對多家媒體報導核實)

| 項目 | 內容 |
|---|---|
| 時間 | Zeabur 於 **2026-08-27** 證實發生資安事件 |
| 成因 | 一組**內部服務憑證**遭未授權存取 |
| 攻擊鏈 | 由一組 **AWS 管理員憑證**入手 → 東京共享叢集 → 控制平面 VPN → 主資料庫 → 取得客戶環境變數 |
| 外洩內容 | 使用者放在專案**環境變數**裡的第三方憑證:OpenAI、Anthropic、OpenRouter、Gemini、GitHub、AWS、Cloudflare、Stripe 等的 API Key 或 Token,以及**資料庫連線字串、密碼、`JWT_Secret`** |
| 實際濫用 | 創辦人林沅霖表示已觀察到 **Anthropic、OpenAI、OpenRouter** 的金鑰遭盜用 |
| 平台回應 | 偵測當日即控制住,涉事憑證已撤銷並輪替,與第三方資安專家調查中;官方表示**無證據顯示帳戶憑證、個資、伺服器資料或付款資訊外洩**;創辦人承諾後續賠償 |

> ⚠️ 影片作者自述本身是受災戶,其中一個專案的 OpenAI 金鑰**被盜刷約一兩百美金**。
> 這是他個人帳單的陳述,本文未也無從獨立查證金額。

### 9.2 三件必須立刻做的事

```mermaid
flowchart TD
    A["① 撤銷並換掉所有金鑰"] --> B["② 開啟花費上限<br/>並確認速率限制"]
    B --> C["③ 關掉自動儲值"]
    C --> D["④ 平時:縮小爆炸半徑<br/>獨立金鑰 + 最小權限"]
```

#### ① 換金鑰 —— 順序錯了等於沒換

⚠️ **最常見的錯誤:只在後台建一把新 Key、填回平台,就以為沒事了。**
**舊的 Key 只要沒有被手動撤銷,駭客照樣能拿著它繼續刷。**

正確順序:

1. 先到**各個服務商後台**把舊 Key **徹底撤銷**
2. 建立全新的 Key
3. 更新平台專案的環境變數,**重新部署**
4. 確認線上服務確實吃到新金鑰
...
- **不要只檢查通知信裡點名的那幾把。** 把**所有曾經放進該平台**的憑證全部盤一遍 —— 包含各家 AI 供應商、資料庫密碼等。
- **換完立刻查各家後台的帳單與用量**:有沒有突然暴增、奇怪的呼叫時間、或出現你平常根本不用的模型。
  發現異常就**先把用量圖表與帳單明細截圖存證**,之後才有東西可以求償。

#### ② 花費上限 ≠ 速率限制(這一段最多人搞混)

| | 管什麼 | 擋得住什麼 |
|---|---|---|
| **速率限制 Rate Limit** | **速度** —— 每分鐘最多幾次請求 | 瞬間灌爆,**擋不住總花費** |
| **硬性花費上限 Hard Spend Limit** | **總額** —— 當月花到上限就停用 API | **真正的荷包防線** |

> 就算限制每分鐘只能呼叫一百次,小偷仍可以**慢速持續呼叫好幾天**;
> 若他挑最貴的頂級模型,累積帳單一樣很可觀。**兩個一起開,風險才控制得住。**

#### ③ 關掉自動儲值(Auto Recharge)

金鑰外洩的情況下,自動儲值等於**幫小偷開無限暢飲** —— 餘額見底就自動刷卡續杯,直到信用卡額度見底。

擔心服務斷線的替代方案:

- 開 **Budget Alert**:餘額低於某金額、或當月花費達上限八成時寄信通知,收到再手動儲值。
- 業務上真的非開不可:**把每次自動儲值金額壓到最低,並設每月儲值次數上限**,讓最壞情況的損失停在可承受範圍。

### 9.3 核心思維:控制爆炸半徑(Blast Radius)

> **爆炸半徑 = 一旦真的出事,最壞會波及多大範圍。**

平時就要養成的習慣:**不同專案、不同環境一律用獨立的 Key,而且每把 Key 只給剛好夠用的權限**
(這就是**最小權限原則**)。很多人習慣一把 Key 到處貼,開發、測試、正式全共用 ——
那等於把整個身家綁在同一條保險絲上。

分開之後,某個小測試專案翻車,燒掉的也只有那個角落。

> 這與 §4① RLS 那一節是同一個思維的兩種應用:**永遠假設外圍防護已經被突破**。
> 影片把它稱為**零信任**:不要假設任何平台百分之百安全,
> 而是從一開始就把「金鑰隨時可能外洩」當成前提設計進系統。

### 9.4 六題資安健檢(5–10 分鐘做得完)

拿出你手上所有 API 服務,逐題回答:

1. 舊的金鑰有沒有定期更換?
2. 你查過平台的**速率限制**嗎?
3. API Key 的**每月花費上限**有沒有設好?
4. 危險的**自動儲值**是不是還開著?
5. **開發與正式環境**有沒有切開?
6. 每把 Key 的**權限**是不是開得太大了?

> **第三方平台什麼時候出意外,我們無法掌控;能掌控的是最壞情況下最多損失多少。**
...
- ⭐ [Zeabur 出事,你今天該做這三件事 — Gary Chen](https://www.youtube.com/watch?v=ExWlrAQrfdc)(2026-08-30,約 5.9 分鐘,**官方 zh-TW 字幕**)—— §9 的三件緊急措施、爆炸半徑與六題健檢來自此片
- Zeabur 事件報導(用於核實 §9.1):
  - [台灣 AI 新創 Zeabur 環境變數外洩!大量使用者 Anthropic、OpenAI、OpenRouter API 金鑰遭盜用 — 電腦王阿達](https://www.kocpc.com.tw/archives/666953)
  - [台灣雲平台 Zeabur 遭駭,用戶 API 金鑰外洩、AI 帳單被盜刷 — 鏈新聞](https://abmedia.io/zeabur-security-incident-api-key-leak-taiwan)
  - [台灣新創 Zeabur 環境變數外洩:AI 金鑰遭盜用,創辦人承諾後續賠償 — 動區動趨](https://www.blocktempo.com/zeabur-environment-variable-leak-openai-anthropic-api-key-stolen-compensation/)
- 核實用的官方資料:
