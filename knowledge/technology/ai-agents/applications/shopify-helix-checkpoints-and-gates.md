...
> ⚠️ **但注意:關卡型 hook **不能**照抄「`stop_hook_active` 為 `true` 就放行」的寫法。**
> `stop_hook_active` 為 `true` 代表「Claude 這次繼續,是因為上次被 Stop hook 擋下」——
> 如果一看到它就放行,關卡**只會擋一次**,第二輪 agent 就能在沒過關的情況下收工。
> ⭐⭐ **關卡型 hook 的終止條件應該是:① 關卡真的通過了;② 自己記的擋下次數到了上限(交給人處理)。**
> **官方在連續擋 8 次沒進展後也會自動覆蓋,作為最後保險。**
> 📎 **本庫 [[claude-code-hooks-complete-guide]] §10.2 有 `stop_hook_active` 的說明與官方寫法 —— 那個寫法適合「只提醒一次」的 hook。**
...
6. ⭐⭐ **「規則是建議,關卡是強制」**:AI LABS 用 **Stop hook 回傳 exit code 2** 讓 agent 不能提早收工(✅ 官方文件核實)。⚠️ **本文補充:要防無限迴圈,但關卡型 hook 不能看到 `stop_hook_active` 就放行(那樣只會擋一次),要靠「關卡真的通過」加上「擋下次數上限」來終止。**
