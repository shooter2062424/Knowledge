...
>
> ⭐⭐ **2026-09-26 增補 YAHA學堂**〈[改个按钮多花40% Token?用Graft给AI装代码地图](https://www.youtube.com/watch?v=9Be7ALZBv0Q)〉(無字幕,faster-whisper 轉錄),成為 **§七:第三條路線 Graft** —— 該片本身就在拆解行銷數字(「上下文少 4 個百分點 ≠ 帳單少 4%」)。⭐⭐ **已 clone Graft 讀 README**,補上三件事:**頂端表格混了兩組不同測試**、**按需查詢(pull)準確率最高達 98%**、**SWE-bench 33 對 27 是廠商自跑而非第三方**。
...
## 七、⭐⭐⭐ 第三條路線:Graft,以及它那幾個百分比該怎麼拆(2026-09-26 增補,來源:YAHA學堂)

> **YAHA學堂**〈[改个按钮多花40% Token?用Graft给AI装代码地图,别再盲目搜文件了](https://www.youtube.com/watch?v=9Be7ALZBv0Q)〉(2026-09-25,約 10.8 分鐘;**無字幕,逐字稿以 CPU faster-whisper 轉錄**)。
>
> ⭐⭐ **這支影片本身就在做本篇 §一的事:把「省力路線」和「行銷數字」分開講。**
> 它其實是在拆解 AI LABS 另一支介紹 Graft 的影片。
>
> ⭐⭐⭐ **本節已 `git clone` [trailhq/Graft](https://github.com/trailhq/Graft)(MIT,TypeScript,本文查詢時 9,222 star)讀 README 核實**,
> ⚠️ **並補上影片與說明欄都沒提的三件事**(見 §7.4)。

---

### 7.1 ⭐⭐ 痛點:改一個按鈕,前面的「找路」比修改本身長得多

> **「你只想把『確認預約』按鈕改成綠色。但 AI 得先搞清楚:按鈕寫在哪?顏色來自這一頁還是公共元件?這個公共元件又被哪些頁面用?」**
>
> ⭐⭐ **「最後也許只改幾行,前面的尋找過程可能比修改本身長得多。」**

📎 **這正是本庫 [[system-one-models-jev-calibrated-decisions]] §16.1 那張表說的:coding agent 的 token 有 30–40% 花在讀檔、10–18% 在搜尋,真正寫程式只占 4–10%。**

### ⭐ 影片先澄清一個常見誤解

> ⚠️ **「上下文變長,不代表每一輪都按完全相同的價格重新收費。」**
> **快取、歷史壓縮、agent 怎麼整理工具輸出,都會影響實際成本;訂閱額度也不能直接拿 token 數去換算。**

---

### 7.2 ⭐⭐ Graft 做什麼(✅ README 核實)

| 項目 | 內容 |
|---|---|
| **解析** | ⭐ **用 tree-sitter 把專案裡的函式、類別、呼叫關係整理成圖**(本篇 §三講的同一套基礎) |
| **儲存** | **圖就是一堆檔案,放在 `graft/`,沒有資料庫、沒有伺服器** |
| **接入 Claude Code** | **`graft init` 會放一個狀態列與 hooks 到 `.claude/`:每則 prompt 自動帶入相關節點,每輪結束後在背景重建圖** |
| ⭐ **不需要金鑰的部分** | **結構圖(`graft build`、`graft check`、`graft ask`)是純 tree-sitter,**不呼叫模型、不連網**** |
| ⚠️ **需要金鑰的部分** | **`graft build --deep` 產生的檔案摘要與每個符號的摘要,要用你自己的模型供應商金鑰** |

> ⭐ **所以影片說「整個工具任何功能都不需要 key,這個說法不準確」—— ✅ 本文核實屬實:只有結構圖不需要。**

### ⭐⭐ 兩種接入方式:提前附加 vs 按需查詢

| 方式 | 做法 | 影片的評估 |
|---|---|---|
| **push(提前附加)** | **你一發出需求,先找相關位置,把線索帶進請求** | **可能少一次來回** |
| **pull(按需查詢,透過 MCP)** | **agent 覺得需要時才呼叫 Graft 的工具** | **可以減少不相關的資訊** |

⭐ **影片說「哪個更合適,要看任務、也要看整合版本」—— 本文在 README 找到了具體數據(見 §7.4 ②)。**

### ⚠️ 畫了連線,不等於找到所有影響

> **動態呼叫、執行期設定、程式碼外面的業務規則,都可能需要額外檢查。
> ⭐ 「地圖幫你定位,最終改得對不對,還要靠原始碼和測試確認。」**

### ⭐⭐⭐ 一個很重要的邊界:程式碼 ≠ 需求

> ⭐⭐⭐ **「程式碼告訴你系統**目前**怎麼運行;需求文件告訴你系統**應該**怎麼運行。」**
>
> **例:「提前 24 小時才能取消預約」這條規則,可能寫在需求文件裡,也可能還沒實作。
> 就算程式碼地圖整理得很清楚,agent 仍然得讀到這條規則,才知道現在要改什麼。**

---

### 7.3 ⭐⭐⭐ 那幾個百分比,拆開來看

**README 頂端寫著「最多便宜 4 倍、快 3 倍」,並列出一組百分比。**
...
> ⭐⭐ **影片的解讀:「四個百分比不一樣,這很正常 —— 呼叫次數、輸入長度、耗時和費用本來就不是同一個指標。」**
> ⚠️ **「『最多便宜 4 倍』是宣傳上限,不能當成每個專案都會拿到的平均結果。」**
> 📌 **本文補充:以上面的平均數字換算,費用 −32% 約等於便宜 1.5 倍,耗時 −60% 約等於快 2.5 倍 —— 離「4 倍、3 倍」還有一段距離。**

### ⚠️⚠️ AI LABS 那次實測:「上下文少 4 個百分點」不等於「帳單少 4%」
...
> ⭐⭐⭐ **影片把這組數字講得很精準:**
> **「耗時差 8 分鐘,以 47 分鐘為基準約減少 17%。上下文占用差 4 個百分點 ——
> 注意,是**上下文占用**,不是帳單減少 4%,也不能直接說少用了 4% 的訂閱額度。」**
> ⚠️ **「這個結果值得參考,但還是**兩次具體執行**。模型輸出會波動,專案狀態、快取和重試都可能影響時間。」**

---

### 7.4 ⚠️⚠️ 本文讀 README 後補上的三件事

#### ① ⚠️⚠️ README 頂端那張表,把兩個不同的測試混在一起

| 來源 | 準確率 | 省下的百分比 |
|---|---|---|
| **自家 162 次測試**(2 個 repo、每題 3 次、Sonnet 5 當 agent、Opus 4.8 當評審) | ⭐ **兩邊都是 93%(相同)** | **−60% / −46% / −42% / −32%** |
| **SWE-bench Verified 50 題** | ⭐ **27/50(54%)→ 33/50(66%)** | **工具呼叫 −25%、token −23%、耗時 −32%** |

> ⚠️⚠️ **頂端表格把「162 次測試的省下百分比」和「SWE-bench 的準確率提升」放在同一張表裡,
> 容易讓人以為「又省 42% token、又多答對 12 分」是同一組測試的結果。實際上是兩組測試各自的結論。**
> ⭐ **README 內文其實有寫清楚:「兩套測試、兩個主張:受控測試說它更便宜更快,SWE-bench 說它也更正確。」**

#### ② ⭐⭐⭐ 「提前附加」和「按需查詢」,README 有數據

> ✅ **README:同一組 162 次測試裡,按需查詢(pull)版本「放棄了大部分速度,換來更重要的東西:**準確率跳到 98%**,比完全不用高 5 分,是整個測試中最強的單項結果」。**
>
> ⭐⭐⭐ **官方自己的建議:「**要速度就用提前附加(push);要正確就用按需查詢(pull)**。」**

📌 **影片只說「要看任務」,這組數據讓判斷有了依據。**

#### ③ ⚠️ SWE-bench 的 33 對 27,是 Graft 自己跑的

**影片說明欄寫:「這是比百分比更硬的**第三方口徑**。」**

> ⚠️ **更精確的說法:**基準是第三方的,但執行者是 Graft 自己**。**
> ✅ README 寫的是「我們也跑了業界標準的 SWE-bench Verified」,用官方評分器、官方映像檔。
> ⭐ **這比自製測試可信(不靠評審模型,補丁直接跑維護者的測試),但仍是 n=50、廠商自跑。**
>
> 📌 **README 另註明:token、費用、呼叫次數的比較**只算兩邊都解出來的題目**。**

#### ④ 兩個使用上要知道的事(README)

| 項目 | 內容 |
|---|---|
| ⭐ **圖不進 git** | ✅ **`graft build` 會自動把 `graft/` 加進 `.gitignore`**;進 git 的是 `.claude/`、`AGENTS.md`、MCP 設定這些接線,**隊友要各自 `graft build`**(✅ 與說明欄一致) |
| ⚠️ **預設會傳匿名使用統計** | **每天一次批次上傳,官方說只含分桶與固定標籤,不含程式碼、路徑、repo 名稱;可用 `graft telemetry disable` 或 `DO_NOT_TRACK=1` 關閉,CI 中預設關閉**(⚠️ 影片沒提) |

⚠️ **立場:Graft 由 Trail 開發,README 同時推廣 Trail 的商業產品。**

---

### 7.5 ⭐⭐⭐ 影片最有價值的一段:裝完怎麼確認它真的在用

**⭐ 三個必查問題:**

| # | 問題 |
|---|---|
| **①** | **設定是不是寫進了正確的專案?**(寫到別的目錄就幫不上忙) |
| **②** | **結構圖是不是真的生成了?**(安裝成功不代表圖已經生成,節點要對得上真實檔案) |
| ⭐⭐ **③** | **agent 有沒有真的用它?** |

> ⭐⭐⭐ **第③題的檢查方法:用一個**範圍明確的定位問題** ——
> 例如「找到確認預約按鈕的處理函式,列出它呼叫的 API,先不要改程式碼」。
> **判斷依據是 agent 的實際呼叫紀錄,以及結果是否跟原始檔案一致。**
> 沒命中就先查版本與設定,**保留報錯和未命中的紀錄**。**

### ⭐ 小專案其實不需要

> **「只有一頁、一個元件的小專案,搜一次可能就找到了,提前建圖能省下的空間有限。」**
> **「專案有很多頁、共用元件和跨檔呼叫,agent 又反覆進來改,才值得觀察這張圖有沒有幫它少走彎路。」**
> ⚠️ **「建圖和維護也有開銷,要看整個工作流程,不能只盯著一次查詢有多快。」**

📎 **這與本篇 §四「不是誰打敗誰,是兩條賽道」、以及 [[graphify-code-knowledge-graph-real-world-test]] 的負面結果一致:**程式碼圖譜的效益取決於專案規模與結構**。**

### ⭐⭐ 自己測的正確做法(影片)

| 做法 |
|---|
| **同一份專案起點、同一個模型、同一條需求,**多跑幾次**** |
| **記下耗時與工具呼叫,**同時檢查最終功能** —— 跑得快但改錯了,不能算更好** |
| **分享結果時寫上專案規模、模型與任務類型,別只留一個「快了多少」** |
...
- ⭐⭐ YAHA學堂(YouTube),〈改个按钮多花40% Token?用Graft给AI装代码地图,别再盲目搜文件了〉(2026-09-25,約 10.8 分鐘;**§七來源**;該片無字幕,逐字稿以 CPU faster-whisper 轉錄):<https://www.youtube.com/watch?v=9Be7ALZBv0Q>
- ⭐⭐⭐ Graft 專案(trailhq,MIT,本文查詢時 9,222 star;**已 clone 讀 README —— §7.4 三處補充的依據**):<https://github.com/trailhq/Graft>
- 延伸(本庫):
